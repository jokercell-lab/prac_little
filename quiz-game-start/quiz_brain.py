class QuizBrain:
    def __init__(self, q_list):
        self.question_number = 0
        self.question_list = q_list
        self.score = 0

    # def next_question(self, text):
    #     next_q = input(f"{text} (True/False)?: ")
    #     print(next_q)
    def still_has_question(self):
        return self.question_number < len(self.question_list) #與直接打出 3 < 5 系統會直接判定為True是相同道理
        # if self.question_number < len(self.question_list): #12
        #     return True
        # else:
        #     return False

    def next_question(self):
        current_q = self.question_list[self.question_number]
        self.question_number += 1
        user_answer = input(f"Q.{self.question_number}: {current_q.text} (True/False)?: ")
        self.check_answer(user_answer, current_q.answer)

    def check_answer(self, user_answer, right_answer):
        if user_answer.lower() == right_answer.lower():
            self.score += 1
            print("you right!")
        else:
            print("Wrong")
        print(f"the answer was {right_answer}.")
        print(f"Your current score is {self.score}/{self.question_number}")
        print("\n")

